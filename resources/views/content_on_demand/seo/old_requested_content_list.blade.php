...
<head>
  @include('user_template.head', ["title"=>"List of Requested Content"])
</head>
...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-md-12">
              <h1 class="m-0">List of Requested Content</h1>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    $(function() {
      ...
      $('#data-tbl').on('click', '.remarks-btn', function() {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/seo/request/content/list/get-remarks`,
            data: {
      ...
      $('#data-tbl').on('click', '.delete-content', function() {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/seo/request/content/list/delete`,
            data: {
              _token: $('#csrf_token_ajax').val(),
              id: val
            },
    ...
    function getContentList() {
      dataTbl.clear().draw();
      let url = `{{url('')}}/ajax/seo/request/content/list`;
    ...
    function setTableData(tbl_id, response) {
      ...
      response.data.forEach((ele, idx) => {
        ...
        let menuEle = `<button class="btn btn-sm btn-link text-danger delete-content" value="${ele.id}">
        ...
        if (ele.status >= 3) {
          contentPreview = `<a href="{{url('')}}/seo/request/content/${ele.id}/preview" target="_blank">Preview</a>`;
        }
