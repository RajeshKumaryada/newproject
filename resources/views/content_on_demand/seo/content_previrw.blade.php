  ...
  <script>
    ...
    $("#form-write-remark").on('submit', function(e) {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/seo/request/content/preview/submit-remark`,
    ...
    const getContentPreview = function() {
      $.ajax({
        type: 'POST',
        url: `{{url('')}}/ajax/seo/request/content/preview`,
        data: {
        ...
        success: function(res) {
          ...
          if (res.code === 200) {
            ...
            //   $('#content-preview').html(`<button class="btn btn-info" href="{{url('')}}/content/user/preview/{${res.data.content_edits.new_content_id}">Download Content File</button>`+res.data.content_edits.content);
            // }
            if(res.data.content_edits.status == 'Approved'){
              $('#content-preview').html(`<a class="btn btn-danger mb-2"  href="{{url('')}}/seo/content/user/preview/${res.data.content_edits.new_content_id}" download>Download Content File</a>`+res.data.content_edits.content);
            }else{
              $('#content-preview').html(res.data.content_edits.content);
            }
