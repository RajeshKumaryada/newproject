...
<body class="hold-transition sidebar-mini layout-fixed">
                ...
                <div class="card-header">
                  <div class="row">
                    ...
                    <div class="col-md-4 text-right">
                      <button type="button" id='save-content' class="btn btn-primary">Save</button>
                      <button type="button" id='draft-content' class="btn btn-primary">Draft</button>
                    </div>
                  </div>
                </div>
  ...
  <script>
    ...
    function wordCount(user_ids,content_ids,counts,timestamp,status){

      var user_id = user_ids;
      var content_id = content_ids;
      var count =counts;
      var time_in_hr_min_sec = timestamp;
      var get_status = status;
      // alert(count);

      $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/content-writer/assigned/edit/add-count-data`,
          data: {
            user_id : user_id,
            content_id: content_id,
            count: count,
            time_in_hr_min_sec : time_in_hr_min_sec, 
    ...
    $(document).bind('keydown', function(e) {
      if (e.ctrlKey && (e.which == 83)) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/content-writer/assigned/edit/save`,
      ...
      $.ajax({
        ...
        success: function(res) {
          ...
          if (res.code === 200) {
            ...
            wordCount(res.data.user_id,res.data.content_id,res.data.count,res.data.hoursArr.total_working_hour_plane,5); 
        ...
        $.ajax({
          ...
          success: (res) => {
            ...
            if (res.code === 200) { 
              ...
              wordCount(res.data.user_id,res.data.content_id,res.data.count,res.data.hoursArr.total_working_hour_plane,4); 
              // window.location.reload();
