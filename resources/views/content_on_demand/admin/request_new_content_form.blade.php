...
<head>
  @include('admin.template.head', ["title"=>"Ask for new Content"])
</head>
                    ...
                    <div class="row">
                      <div class="col-md-8">
                        <div class="form-group">
                          ...
                          <select class="form-control select2bs4" name="request_by" id="request_by">
                            <option value="">--Select--</option>
                            @foreach($users as $row)
                            <option value="{{$row->user_id}}">{{$row->username}} - {{$row->empPost->post_name}}</option>
                            @endforeach
                          </select>
                        </div>
                      </div>
  ...
  <script>
    ...
    $("#content-demand-form").on('submit', function(e) {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/admin/request/content/new`,
