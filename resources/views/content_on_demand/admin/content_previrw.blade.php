...
<head>
  @include('admin.template.head', ["title"=>"Content Preview for SEOs"])
</head>
  ...
  <script>
    ...
    $("#form-write-remark").on('submit', function(e) {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/admin/request/content/preview/submit-remark`,
    ...
    const getContentPreview = function() {
      $.ajax({
        type: 'POST',
        url: `{{url('')}}/ajax/admin/request/content/preview`,
        data: {
