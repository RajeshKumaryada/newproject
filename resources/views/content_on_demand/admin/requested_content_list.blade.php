...
<head>
  @include('admin.template.head', ["title"=>"Writer Leader - Requested Content List"])
</head>
...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-md-12">
              <h1 class="m-0">Requested Content List</h1>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    $(function() {
      ...
      $('#data-tbl').on('click', '.remarks-btn', function() {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/request/content/list/get-remarks`,
            data: {
      ...
      $('#data-tbl').on('click', '.delete-content', function() {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/request/content/list/delete`,
            data: {
              _token: $('#csrf_token_ajax').val(),
              id: val
            },
      ...
      $('#data-tbl').on('click', '.approve-content', function() {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/request/content/list/approve`,
            data: {
              _token: $('#csrf_token_ajax').val(),
              id: val
            },
    ...
    function getContentList() {
      dataTbl.clear().draw();
      let url = `{{url('')}}/ajax/admin/request/content/list`;
      ...
      response.data.forEach((ele, idx) => {
        ...
        let menuEle = `<div class="btn-group text-center">
            ...
            <div class="dropdown-menu" role="menu">
              <a class="dropdown-item" href="{{url('')}}/admin/request/content/${ele.id}/assign"><i class="fas fa-user"></i> Assign</a>
              ...
              <button class="dropdown-item btn btn-sm btn-link text-danger delete-content" value="${ele.id}">
        ...
        if (ele.status >= 3) {
          contentPreview = `<a href="{{url('')}}/admin/request/content/${ele.id}/preview" target="_blank">Preview</a>`;
        }
    ...
    //   window.open(`{{url('')}}/admin/request/content/${id}/assign`, '_self');
    // });
  </script>
