...
<head>
  @include('admin.template.head', ["title"=>"Order Month Report"])
</head>
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-12">
              <h1 class="m-0">Order Month Report</h1>
            </div>
            ...
            <div class="col-md-12">
              ...
              <div class="card card-primary">
                <div class="card-header">
                  <h3 class="card-title">Order Month Report</h3>
                </div>
                <!-- /.card-header -->
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-3">
                            <div class="form-group">
                              ...
                              <input type="month" name="start_date" id="start_date" value="{{date('Y-m')}}" class="form-control" placeholder="Enter ...">
                            </div>
                          </div>
                          ...
                          <div class="col-md-3">
                            <div class="form-group">
                              ...
                              <input type="month" name="end_date" id="end_date" value="{{date('Y-m')}}" class="form-control" placeholder="Enter ...">
                            </div>
                          </div>
                    ...
                    <div class="col-md-12 mb-4">
                      <div class="row">
                        <div class="col-md-8 small">
                          ...
                          <button class="btn btn-sm btn-link text-danger"><i class="fas fa-calendar-alt"></i></button>: By Date
                        </div>
  ...
  <script>
    ...
    $(frmEle).on('submit', function(e) {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/admin/order/report/month-view`,
    ...
    $('#data-tbl tbody').on('click', 'button.more-info-details', function() {
      ...
      if (row.child.isShown()) {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/order/report/month-view/details`,
            data: {
    ...
    $('#data-tbl tbody').on('click', 'button.more-info-users', function() {
      ...
      if (row.child.isShown()) {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/order/report/month-view/details-users`,
            data: {
              _token: $('#csrf_token_ajax').val(),
              date: orderDate
            },
    ...
    $('#data-tbl tbody').on('click', 'button.more-info-sites', function() {
      ...
      if (row.child.isShown()) {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/order/report/month-view/details-sites`,
            data: {
              _token: $('#csrf_token_ajax').val(),
              date: orderDate
            },
    ...
    /**
     * getting more date wise details
     */
    $('#data-tbl tbody').on('click', 'button.more-info-date', function() {
      ...
      if (row.child.isShown()) {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/order/report/month-view/details-date`,
            data: {
              _token: $('#csrf_token_ajax').val(),
              date: orderDate
            },
            ...
            success: (res) => {
              ...
              if (res.code === 200) {
                ...
                if (res.data.length > 0) {
                  setTableDataDate(taskInfoTbl, res, this);
                } else {
                  Toast.fire({
                    icon: 'error',
                    title: 'No data found'
    ...
    $('#data-tbl tbody').on('click', 'button.users-sites-info', function() {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/admin/order/report/month-view/details-sites-users`,
          data: {
            _token: $('#csrf_token_ajax').val(),
            date: orderDate,
            userid: userId,
      ...
      response.data.forEach((ele, idx) => {
        ...
        menu = `
          <div class="text-center">
            <button class="btn btn-sm btn-link text-danger more-info-details" data-month="${ele.ord_month}">
              <i class="fas fa-plus"></i>
            </button>
            <button class="btn btn-sm btn-link text-danger more-info-users" data-month="${ele.ord_month}">
              <i class="fas fa-user"></i>
            </button>
            <button class="btn btn-sm btn-link text-danger more-info-sites" data-month="${ele.ord_month}">
              <i class="fab fa-internet-explorer"></i>
            </button>
            <button class="btn btn-sm btn-link text-danger more-info-date" data-month="${ele.ord_month}">
              <i class="fas fa-calendar-alt"></i>
            </button>
          </div>`;

        // menu = `<div class="text-center">
        //     <button class="btn btn-sm btn-danger more-info-details" data-month="${ele.ord_month}">More +</button>
        //     <button class="btn btn-sm btn-danger more-info-users" data-month="${ele.ord_month}">Users +</button>
        //     <button class="btn btn-sm btn-danger more-info-sites" data-month="${ele.ord_month}">Sites +</button>
        //     <button class="btn btn-sm btn-danger more-info-date" data-month="${ele.ord_month}">Date +</button>
        //     </div>`;
