...
<head>
  @include('admin.template.head', ["title"=>"Edit SEO Team"])
</head>
...
<body class="hold-transition sidebar-mini layout-fixed">
            ...
            <div class="col-md-12">
              ...
              <div class="card card-primary">
                ...
                <form id="add-user-form">
                  <div class="card-body">
                    @csrf()
                    <input type="hidden" name="team_id" value="{{$teamInfo->wt_id}}">
                    <div class="row">

                      <div class="col-md-6">
                        <div class="form-group">
                          <label>Team Name [Team name should be unique]</label>
                          <input type="text" name="team_name" id="team_name" value="{{$teamInfo->team_name}}" class="form-control cap-case">
                        </div>
                      </div>
  ...
  <script>
    ...
    function getTeamList(dataTbl) {
      dataTbl.clear().draw();
      let url = `{{url('')}}/ajax/admin/seo-team/manage/member-list/{{$team_id}}`;
    ...
    function setTableData(tbl_id, response) {
      ...
      response.data.forEach((ele, idx) => {
        ...
        menuEle = `<div class="text-center">
            <span class="user-badge text-danger"><i data-id="${ele.wtm_id}" class="fas fa-trash cur-ptr delete-team"></i></span>
          </div>`;

        // ele.work_team_members.forEach(row => {
        if (ele.member_type === "team_leader") {
          teamMembers += `<span class="badge badge-danger user-badge" title="Team Leader">Team Leader</span>`;
        } else {
          teamMembers += `<span class="badge badge-success user-badge" title="Team Member">Team Member</span>`;
        }
    ...
    $(frmEle).on('submit', function(e) {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/admin/seo-team/manage/update`,
    ...
    $('#data-tbl').on('click', '.delete-team', function() {
      ...
      $.ajax({
        type: 'POST',
        url: `{{url('')}}/ajax/admin/seo-team/manage/member-delete`,
        data: {
          member_id: teamId,
