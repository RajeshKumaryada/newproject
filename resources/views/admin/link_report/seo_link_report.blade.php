...
<head>
  @include('admin.template.head', ["title"=>"SEO Link Report (Count)"])
  ...
  <script>
    ...
    $(frmEle).on('submit', function(e) {
      ...
      if (!isAjax) {
        ...
        $.ajax({
          type: 'POST',
          url: `{{url('')}}/ajax/admin/work-report/seo-link-count`,
    ...
    function setTableData(tbl_id, response) {
      ...
      $('#data-table-work-report').on('click', '.links-more-info', function() {
        ...
        if (!isAjax) {
          ...
          $.ajax({
            type: 'POST',
            url: `{{url('')}}/ajax/admin/work-report/seo-link-details`,
            data: {
    ...
    function setTableDataMore(ele, response) {
      ...
      // Object.keys(response.data_task_links).forEach((row, idx) => {
      //   console.log(row, idx);
      //   tableEle += `<tr>
      //       <td><span class="text-danger">${idx + 1}</span></td>
      //       <td scope="col">${response.data_task_links.row.task_name}</td>
      //       <td scope="col">${response.data_task_links.row.task_count}</td>
      //       </tr>`;
      // });
