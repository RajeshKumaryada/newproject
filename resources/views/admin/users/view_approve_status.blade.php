  ...
  <script>
    ...
    function setTableData(tbl_id, response) {
      ...
      response.data.bank.forEach((ele, idx) => {
        ...
        var str = `${ele.passbook}`;
         var fl = str.substr(-3);

         if(fl === 'pdf'){
          images_passbook = `<a href="{{url('')}}/users/employee-documents/images/${ele.passbook}" download>
                        Link
                      </a>`;
           
         }else{

          images_passbook = `<a href="{{url('')}}/users/employee-documents/images/${ele.passbook}" data-toggle="lightbox" data-title="Passbook" data-gallery="gallery">
                        Link
                      </a>`;
         }
