...
class UsersApproveStatusDocCtrl extends Controller
{
    ...
    public function fetchUsersDocTemp()
    {
        ...
        $usersAddressTemp = UserAddressTemp::with('userName:username,user_id')->get();

        $return['code'] = 200;
        $return['msg'] = 'Data Found';
        $return['data'] = ['doc' => $usersDocTemp, 'bank' => $usersBankInfoTempDoc, 'address' => $usersAddressTemp];
    ...
    public function approveAddress(Request $request)
    {
        ...
        $userAddressinsert = UserAddressTemp::find($id);
        //   dd($type);
        if ($status == 1) {

            $userAddressoriginal = UserAddress::where('id', $type)->first();

            $userAddressoriginal->address_type = $userAddressinsert->address_type;
            $userAddressoriginal->address_line_one = $userAddressinsert->address_line_one;
            $userAddressoriginal->address_line_two     = $userAddressinsert->address_line_two;
            $userAddressoriginal->city = $userAddressinsert->city;
            $userAddressoriginal->state = $userAddressinsert->state;
            $userAddressoriginal->country = $userAddressinsert->country;
            $userAddressoriginal->postal_address = $userAddressinsert->postal_address;

            if ($userAddressoriginal->save()) {
                $return['code'] = 200;
                $return['msg'] = "Bank Info Approved";
            }
            ...
            $userAddressinsert->status = 2;
            $userAddressinsert->remark = $remark;

            if ($userAddressinsert->save()) {

                $return['code'] = 200;
                $return['msg'] = "Address Deny";
                $return['data'] = $userAddressinsert;
            }
        } else {
            $return['code'] = 200;
            $return['msg'] = "Not Found";
     ...
     * Update Address
     */
    public function updateAddressRemark(Request $request)
    {
        $validator = Validator::make(
            $request->all(),
            [
        ...
        if (!empty($id)) {
            $remark = $request->remark;
            $userinsert = UserAddressTemp::find($id);
            $userinsert->status = 2;
            $userinsert->remark = $remark;
            ...
            if ($userinsert->save()) {
                ...
                $return['code'] = 101;
                $return['msg'] = 'Remark Not Inserted';
            }
    ...
    /**
     * Fetch Users Address  Remark
     */
    public function fetchUsersAddressRemark(Request $request)
    {
        $id = $request->add_id;
        $dataaddressRemark = UserAddressTemp::find($id);
        $return['code'] = 200;
        $return['msg'] = "Data Found";
        $return['dataAddremark'] = $dataaddressRemark;
        return response()->json($return);
    }
