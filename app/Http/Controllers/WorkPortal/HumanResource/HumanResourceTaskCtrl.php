...
class HumanResourceTaskCtrl extends Controller
{
  ...
  public function addTask(Request $request)
  {
    ...
    $taskCount = HumanResourceTask::where('user_id', $trackSession->userId())
      ->where('complete', 0)
    ...
    if ($complete == 0) {
        ...
        if ($devtask->save()) {
            ...
            $return['code'] = 200;
            $return['msg'] = 'Start Task successfully.';
            $return['data'] = $alarm;
        } else {
            $return['code'] = 101;
  ...
  public function finishTask(Request $request)
  {
    ...
    // $trackSession = unserialize($trackSession);
    ...
    $devtask = HumanResourceTask::find($task_id);
  ...
  public function getTodayTaskList(Request $request)
  {
    ...
    //check if running task available
    $taskList = HumanResourceTask::where('user_id', $trackSession->userId())
      ->whereDate('start_time', date("Y-m-d"))
      ->orderBy('id', 'DESC')
      ->get();
