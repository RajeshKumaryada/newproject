...
class DeveloperTaskCtrl extends Controller
{
  ...
  public function addTask(Request $request)
  {
    ...
    $taskCount = DeveloperTask::where('user_id', $trackSession->userId())
      ->where('complete', 0)
  ...
  public function finishTask(Request $request)
  {
    ...
    $devtask = DeveloperTask::find($task_id);
  ...
  public function getTodayTaskList(Request $request)
  {
    ...
    //check if running task available
    $taskList = DeveloperTask::where('user_id', TrackSession::get()->userId())
      ->whereDate('start_time', date("Y-m-d"))
      ->orderBy('id', 'DESC')
      ->get();
