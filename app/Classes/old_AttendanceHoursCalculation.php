...
class AttendanceHoursCalculation
{
  ...
  public function calculateContentTimestamp($times,$crrDateNew = null)
  {
    ...
    foreach ($times as $key => $rows_newd) {
      ...
      //   unset($times[$key]);
      // }
    // Time difference from start to end  
    ...
    $sd = [];
     foreach($list as $keys => $value){
      //  $s = array_search("",$list);
      // dd($value);
        // $keyyData = date('Y-m-d',strtotime($keys));
        

        if($keys == $crrDateNew){
         $r[] = date('H:i:s',strtotime($value));
        //  $date = date_create($s);
        //   $hr =  date_format($date,"H");
        //   $min =  date_format($date,"i");
        //   $sec =  date_format($date,"s");
